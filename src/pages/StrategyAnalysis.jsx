...
const StrategyAnalysis = () => {
    ...
    const [selectedKAUStrategyId, setSelectedKAUStrategyId] = useState("");
            ...
            <div class="p-3 bg-white border border-gray-200 rounded-lg shadow-lg">
              ...
              <div class="space-y-1 text-sm">
                <div class="flex justify-between">
                  ...
                  <span class="font-medium">${(data.investment / 1000000).toFixed(1)}백만원</span>
                </div>
                <div class="flex justify-between">
                  <span class="text-gray-600">감축률:</span>
                  <span class="font-medium text-green-600">${data.y.toFixed(1)}%</span>
                </div>
                <div class="flex justify-between">
                  <span class="text-gray-600">효율성 지수:</span>
                  <span class="font-medium text-blue-600">${data.efficiency.toFixed(1)}</span>
                </div>
              </div>
            </div>
          `;
            ...
            markers: {
                size: efficiencyData.map((d) => Math.max(6, Math.min(12, d.efficiency / 10))),
                colors: ["#10B981"],
                strokeColors: ["#059669"],
                strokeWidth: 2,
            },
    ...
    useEffect(() => {
        if (!apexChartsLoaded)
            return;
        const comparisonChartElement = document.getElementById("kau-comparison-chart");
            ...
            tooltip: {
                ...
                custom: function ({ series, seriesIndex, dataPointIndex, w }) {
                    if (seriesIndex === 0) {
                        ...
                        return `
               <div class="p-3 bg-white border border-gray-200 rounded-lg shadow-lg">
                 <div class="font-semibold text-gray-900 mb-2">${strategy.name}</div>
                 <div class="space-y-1 text-sm">
                   <div class="flex justify-between">
                     <span class="text-gray-600">투자금액:</span>
                     <span class="font-medium">${(strategy.investment / 1000000).toFixed(1)}백만원</span>
                   </div>
                   <div class="flex justify-between">
                     <span class="text-gray-600">감축률:</span>
                     <span class="font-medium text-green-600">${strategy.reductionRate}%</span>
                   </div>
                   <div class="flex justify-between">
                     <span class="text-gray-600">ROI:</span>
                     <span class="font-medium text-blue-600">${strategy.roi}%</span>
                   </div>
                 </div>
               </div>
             `;
                        ...
                        return `
               <div class="p-3 bg-white border border-gray-200 rounded-lg shadow-lg">
                 <div class="font-semibold text-gray-900 mb-2">${kauStrategy.name}</div>
                 <div class="space-y-1 text-sm">
                   <div class="flex justify-between">
                     <span class="text-gray-600">투자금액:</span>
                     <span class="font-medium">${(kauStrategy.totalCost / 1000000).toFixed(1)}백만원</span>
                   </div>
                   <div class="flex justify-between">
    ...
    const selectedKAUStrategyData = kauStrategies.find((k) => k.id === selectedKAUStrategyId);
    return (<div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {strategies.map((strategy) => (<div key={strategy.id} className={`bg-white border rounded-lg p-6 shadow-sm cursor-pointer transition-all duration-200 hover:shadow-md ${selectedScenario === strategy.id
                ? "border-green-500 bg-green-50"
                : "border-gray-200 hover:border-green-300"}`} onClick={() => setSelectedScenario(strategy.id)}>
      ...
      {/* 선택된 전략 상세 분석 */}
      {selectedStrategy && (<div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
            ...
            <div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between">
                  ...
                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${getRiskColor(selectedStrategy.risk)}`}>
      ...
      <div className="bg-gradient-to-r from-red-50 to-orange-50 border border-red-200 rounded-lg p-6 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          {kauStrategies.map((strategy) => (<div key={strategy.id} className={`bg-white border rounded-lg p-4 shadow-sm cursor-pointer transition-all duration-200 hover:shadow-md ${selectedKAUStrategyId === strategy.id
                ? "border-red-500 bg-red-50"
                : "border-gray-200 hover:border-red-300"}`} onClick={() => setSelectedKAUStrategyId(strategy.id)}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {/* 선택된 KAU 전략 상세 분석 */}
          {selectedKAUStrategyData ? (<div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
              <div className="flex items-center justify-between mb-4">
              ...
              <div className="grid grid-cols-1 gap-4">
                ...
                : selectedKAUStrategyData.marketCondition ===
                    ...
                    <div className="flex justify-between">
                      <span className="text-gray-600">리스크 수준</span>
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${getRiskColor(selectedKAUStrategyData.risk)}`}>
                ...
                selectedKAUStrategyData.quantity).toLocaleString()}
                        원
                      </span>
                    </div>
                  </div>
            ...
            </div>) : (<div className="bg-gray-50 border border-gray-200 rounded-lg p-6 shadow-sm flex items-center justify-center">
              <div className="text-center">
