...
const MarketChart = ({ data, showStats = true, showBuySignals = false, }) => {
    ...
    const buySignals = useMemo(() => {
        ...
        for (let i = 50; i < data.length; i++) {
            ...
            const recentPrices = data
                ...
                .map((d) => d.close);
            const mean = recentPrices.reduce((sum, price) => sum + price, 0) /
                recentPrices.length;
            ...
            else if (currentVolume > avgVolume * 2 && // 거래량 2배 이상
            ...
            else if (currentPrice > prevPrice * 1.02 && // 2% 이상 상승
                currentPrice >
            ...
            // 6. 가격 급등 - 보수적 조건
            else if (currentPrice > prevPrice * 1.08 && // 8% 이상 상승
            ...
            // 7. 거래량 급증 - 보수적 조건
            else if (currentVolume > avgVolume * 3 && // 거래량 3배 이상
                currentPrice > prevPrice * 1.02 && // 2% 이상 상승
                currentPrice >
    ...
    useEffect(() => {
        ...
        console.log("매수 신호 annotations:", showBuySignals
            ? buySignals.map((signal) => ({
                x: new Date(signal.date).getTime(),
                y: signal.price,
            ...
            annotations: showBuySignals
                ? {
                    points: buySignals.map((signal, index) => ({
                        ...
                        y: signal.price,
                        marker: {
                            size: signal.strength === "strong"
                                ? 8
                                : signal.strength === "medium"
                                    ? 6
                                    : 4,
                            fillColor: signal.strength === "strong"
                                ? "#10b981"
                                : signal.strength === "medium"
                                    ? "#f59e0b"
                                    : "#6b7280",
                        ...
                        label: {
                            borderColor: signal.strength === "strong"
                                ? "#10b981"
                                : signal.strength === "medium"
                                    ? "#f59e0b"
                                    : "#6b7280",
                            ...
                            style: {
                                color: "#ffffff",
                                background: signal.strength === "strong"
                                    ? "#10b981"
                                    : signal.strength === "medium"
                                        ? "#f59e0b"
                                        : "#6b7280",
    ...
    return (<div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
      ...
      {/* 통계 정보 */}
      {showStats && stats && (<div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
      ...
      {/* 매수 신호 정보 */}
      {showBuySignals && buySignals.length > 0 && (<div className="mt-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {buySignals
                .slice(-10)
                .reverse()
                .map((signal, index) => (<div key={index} className={`rounded-lg p-4 border ${signal.strength === "strong"
                  ...
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${signal.strength === "strong"
